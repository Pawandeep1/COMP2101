...
ttps://zonzorp.net/pics.tgz tarfileound $(find ~/public_html/pics -type f|wc -l) files in the public_html/pics directory.
The public_html/pics directory uses $(du -sh ~/public_html/pics|awk '{print $1}') space on the disk.
EOF 
wget -q -O ~/public_html/pics/pics.tgz https://zonzorp.net/pics.tgz tarfile &&  rm $tarfile
